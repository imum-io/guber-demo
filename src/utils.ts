...
import { currentTimeZone, dbServers, invalidDate, vehicleTypes } from './config/enums'
...
function matchLabelTranslation(label: string, translation: any, caseSensitive: boolean = false) {
    for (const property in translation) {
        let value = caseSensitive ? translation[property] : translation[property]?.toLowerCase()
...
function formatToDatePredefined(value, format?) {
    ...
    let formatted = momentTz(value).tz(currentTimeZone).format(format || 'YYYY-MM-DD HH:mm:ss')
...
function formatTimeToLTZone(format?: string) {
    return momentTz().tz(currentTimeZone).format(format ?? 'YYYY-MM-DD HH:mm:ss')
}
...
function getDbserver(productType: string) {
    if (productType == vehicleTypes.trailer || productType == vehicleTypes.truck) {
        return dbServers.trucks
    }
    else if (productType == vehicleTypes.household || productType == vehicleTypes.realestateProject) {
