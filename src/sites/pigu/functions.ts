...
export class PGUFunctions implements HomeAppliancesInterface {
        ...
        $('#productListLoader')
            ...
            .each((i, element) => {
                if ($(element).attr('widget-data')) {
                    const widgetData: WigdetData = JSON.parse(
                        $(element).attr('widget-data')
                    )
                    if (widgetData?.url && widgetData?.productId) {
                        let url = widgetData.url
                        if (!isValidURL(widgetData.url))
                            url = getBaseUrl(widgetData.ua) + widgetData.url
    ...
    public scrapeHomeAppliancesItem(
        $: CheerioAPI,
        url: string
    ): HomeAppliencesType<string> {
        ...
        $('script').each((i, elem) => {
            if (
                (elem.children[0] as any)?.data.match(
                    /\"productBarcode\":\s*\"(.*?)\",/
                )?.[1]
            ) {
                item.barcode = (elem.children[0] as any)?.data.match(
                    /\"productBarcode\":\s*\"(.*?)\",/
                )?.[1]
            }
        })
        ...
        let primaryPrice =
            productPrices.match(/\"sell_price\":\s*\"(.*?)\",/)?.[1] ?? '0'

        let secondaryPriceEl = $(
            '.c-price.h-price--small',
        ...
        let secondaryPricefloatEl = $(
            '.c-price.h-price--small sup',
            '.c-product__price-box'
        )
            ?.contents()
            .text()
        ...
        item.inStock = Boolean(
            $('div[data-cy="product-page-add-to-cart"]').length
        )
