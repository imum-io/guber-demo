...
export async function assignBrandIfKnown(countryCode: countryCodes, source: sources, job?: Job) {
    ...
    // Step 1: Create groups of related brands
    const brandGroups: Map<string, Set<string>> = new Map()
    for (const brandKey in brandsMapping) {
        const relatedBrands = brandsMapping[brandKey]
        const normalizedBrandKey = normalizeBrand(brandKey)

        if (!brandGroups.has(normalizedBrandKey)) {
            brandGroups.set(normalizedBrandKey, new Set())
        }

        relatedBrands.forEach((brand) => {
            const normalizedBrand = normalizeBrand(brand)
            brandGroups.get(normalizedBrandKey)!.add(normalizedBrand)
        })
    }

    // Step 2: Choose representatives for each group
    const brandRepresentatives: Map<string, string> = new Map()
    brandGroups.forEach((relatedBrands, brandKey) => {
        const representative = Array.from(relatedBrands)[0]
        relatedBrands.forEach((brand) => {
            brandRepresentatives.set(brand, representative)
        })
    })
    ...
    for (let product of products) {
        ...
        // Step 3: Normalize and deduplicate matched brands
        const normalizedMatchedBrands = matchedBrands.map((brand) => {
            const normalizedBrand = normalizeBrand(brand)
            return brandRepresentatives.get(normalizedBrand) || normalizedBrand
        })
        const uniqueMatchedBrands = _.uniq(normalizedMatchedBrands)
    
        // Step 4: Sort by position in title
        uniqueMatchedBrands.sort((a, b) => {
            const indexA = product.title.toLowerCase().indexOf(a.toLowerCase())
            const indexB = product.title.toLowerCase().indexOf(b.toLowerCase())
            if (indexA === -1) return 1
            if (indexB === -1) return -1
        ...
        const meta = { matchedBrands: uniqueMatchedBrands }
